...
exports.prepareSimulation = function(ctx)
{
	CreateLandmass(ctx);
	CreateOcean(ctx, ctx.area * 25); //  Each tile would get 25
};

function CreateLandmass(ctx)
{
    var continentMap = new Array(ctx.area);
    for(var z = 0 ; z < ctx.area; z++) continentMap[z]=0;
    ...
    for (var c = 0; c < 3; c++)
    {
        ...
        while (centerContinentX == -1 || centerContinentY == -1)
        {
            ...
            var attemptedY = (ctx.rows / 2) + utility.randomNumberBetween(-ctx.rows / 3, ctx.rows / 3);
        ...
        for (var i = 0; i < 3; i++)
        {
            var blobX = centerContinentX + utility.randomNumberBetween(-radius,radius);
            var blobY = centerContinentY + utility.randomNumberBetween(-radius,radius);
            var blob_radius = radius / (utility.randomNumberBetween(0,3) + 1);
            
            var blobResults = ctx.GetRingOfCoordinates(ctx.ConvertToZ({f:blobX,s:blobY}), blob_radius, true);

            for (var p = 0; p < blobResults.length; p++)
            {
                if(continentMap[blobResults[p]]!=1)
                { 
                    ctx.height[blobResults[p]] = 5 + utility.randomNumberBetween(0,5);
