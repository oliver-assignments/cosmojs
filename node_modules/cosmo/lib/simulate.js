...
exports.simulateDay = function(ctx,res)
{
    for(var p = 0 ; p < ctx.plantArea ; p++)
    {
        ...
        if(ctx.depth[ctx.ConvertPToZ(p)] > 1)
        {
        ...
        if(ctx.hasPlant[p])
        {
            ctx.nutroStore[p] += 2;//ctx.GetNutroConsumption(p) - ctx.GetNutroMetabolism(p);
            ctx.nuciumStore[p] += 2;//ctx.GetNuciumConsumption(p) - ctx.GetNuciumMetabolism(p);

            if( ctx.nutroStore[p] > 6//ctx.GetNutroEndowment(p) 
                && 
                ctx.nuciumStore[p] > 6)//ctx.GetNuciumEndowment(p))
            {
                var neighbors = ctx.GetPlantNeighbors(p,true);
                for(var q = 0 ; q < neighbors.length ; q++)
                {
                    var neighbor = neighbors[q];    
                    
                    if(!ctx.hasPlant[neighbor])
                    {
                        // Plant something!
                        ...
                        ctx.dna[neighbor] = 'acadcd'//ctx.dna[p];//MutateDNA(ctx.dna[p]);
                        ctx.nutroStore[neighbor] = 1;
                        ctx.nuciumStore[neighbor] = 1;
                        ctx.hasPlant[neighbor] = true;
                    }
                }   
            } 
        }
            ...
            //  Have we run out of a resource and died?
            // if(ctx.nutroStore[p] < 0 || 
            //     ctx.nuciumStore[p] < 0)
            // {
            //     // console.log('death');
            ...
            //     // ctx.nutroCurrent[p] += ctx.GetNutroEndowment[p];
            //     // ctx.nuciumCurrent[p] += ctx.GetNuciumEndowment[p];
            //     continue;
            // }

            // //  Nucium consumption
            // if(ctx.nutro[p] - ctx.GetNutroConsumption(p) >= 0)
            // {
            //     ctx.nutroCurrent[p] -= ctx.GetNutroConsumption(p);
                
                    //console.log("Consumed " + ctx.GetNutroConsumption(p) + " from a plot that had " + ctx.nutro[p] + " but metabolized "+ ctx.GetNutroMetabolism(p) + ".");
                    // ctx.nutroStore[p] += ctx.GetNutroConsumption(p) - ctx.GetNutroMetabolism(p);
            // }
            // else
            // {
            //     //console.log("Consumed all from a plot that had " + ctx.nutro[p] + " but metabolized "+ ctx.GetNutroMetabolism(p) + ".");
            //     ctx.nutroStore[p] += 
            //         ctx.nutroCurrent[p] - ctx.GetNutroMetabolism(p);
            //     ctx.nutro[p] = 0;
            // }

            // //  Nucium consumption
            // if(ctx.nucium[p] - ctx.GetNuciumConsumption(p) >= 0)
            // {
            //     ctx.nuciumCurrent[p] -= ctx.GetNuciumConsumption(p);

                    // ctx.nuciumStore[p] += ctx.GetNuciumConsumption(p) - ctx.GetNuciumMetabolism(p);
            // }
            // else
            // {
            //     ctx.nuciumStore[p] += 
            //         ctx.nuciumCurrent[p] - ctx.GetNuciumMetabolism(p);
            //     ctx.nucium[p] = 0;
            // }

            //  Have we surplus enough to seed?
            // if(ctx.nutroStore[p] > ctx.GetNutroEndowment(p) && ctx.nuciumStore[p] > ctx.GetNuciumEndowment(p))
            // {
            //     var neighbors = ctx.GetPlantNeighbors(p,true);
            ...
            //     {
            //         var neighbor = neighbors[q];    
            //         var z = ctx.ConvertPToZ(neighbor);

            //         if(ctx.depth[z] == 0 && !ctx.hasPlant[neighbor])
            //         {
            //             // Plant something!
            //             ctx.nutroStore[p] -= ctx.GetNutroEndowment(p);
            //             ctx.nuciumStore[p] -= ctx.GetNuciumEndowment(p);
            ...
            //             //  This needs to be made pending!
            //             ctx.dna[neighbor] = 'acadcd'//ctx.dna[p];//MutateDNA(ctx.dna[p]);
            //             ctx.nutroStore[neighbor] = ctx.GetNutroEndowment(p);
            //             ctx.nuciumStore[neighbor] = ctx.GetNuciumEndowment(p);
            //             ctx.hasPlant[neighbor] = true;
            //         }
            //     }
