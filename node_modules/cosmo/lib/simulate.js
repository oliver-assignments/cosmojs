...
exports.simulateDay = function(ctx,res)
{
    ...
    for(var q = 0 ; q < ctx.plantArea ; q++)
    {
        ...
        //  Seeding
        if
            (ctx.nutroStore[p] > ctx.GetNutroEndowment(p) * ctx.GetNumberSeeds(p)
            && 
            ctx.nuciumStore[p] > ctx.GetNuciumEndowment(p) * ctx.GetNumberSeeds(p)) 
        {
            ...
            for(var c = 0 ; c < 3 ; c++)
            {
                ...
                for(var s = 0 ; s < ctx.GetNumberSeeds(p) ; p++)
                {
