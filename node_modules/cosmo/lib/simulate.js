...
exports.simulateDay = function(ctx,res)
{
    for(var p = 0 ; p < ctx.plantArea ; p++)
    {
        ...
        {
            ctx.nucium[p] -= ctx.GetNuciumConsumption(p);

            ctx.nuciumStore[p] += 
                ctx.GetNuciumConsumption(p) - ctx.GetNuciumMetabolism(p);
        }
        else
        {
            ctx.nuciumStore[p] += 
                ctx.nucium[p] - ctx.GetNuciumMetabolism(p);
            ctx.nucium[p] = 0;
        }

        //  Do we have surlus enoguh to plant?
        if(ctx.nutroStore[p] > ctx.GetNutroEndowment(p) && 
            ctx.nuciumStore[p] > ctx.GetNuciumEndowment(p))
        {
            ...
            for(var n = 0 ; n < neighbors.length ; n++)
            {
                if(!ctx.hasPlant[neighbors[n]])
                {
                    ...
                    ctx.dna[neighbors[n]] = MutateDNA(ctx.dna[p]);
                    ctx.nutroStore[neighbors[n]] = ctx.GetNutroEndowment(p);
                    ctx.nuciumStore[neighbors[n]] = ctx.GetNuciumEndowment(p);
                    ctx.hasPlant[p] = true;
                    break;
                }
            }
            ...
            ctx.nuciumStore[p] < 0)
        {
            ...
            ctx.nutro[p] += ctx.GetNutroEndowment[p]*2;
            ctx.nucium[p] += ctx.GetNuciumEndowment[p]*2;
        }
    }
    res(null);
    //console.log("A day! " + ctx.day);
};
...
function CreateBlob(ctx,radius,numberNodes)
{
...
function SprayPlants(ctx)
{
    for (var p = 0; p < ctx.plantArea; p++)
    {
        ...
        if(Math.random()>0.8)
        {
            ...
            // console.log(
            //     "Nutro Metabolism: " + 
            //     ctx.GetNutroMetabolism(p) +
            //     ", Nutro Consumption: " +
            //     ctx.GetNutroConsumption(p) +
            //     ", Nutro Endowment: " +
            //     ctx.GetNutroEndowment(p)+
            //     ", Nucium Metabolism: " +
            //     ctx.GetNuciumMetabolism(p)+
            //     ", Nucium Consumption: " +
            //     ctx.GetNuciumConsumption(p)+
            //     ", Nucium Endowment: " +
            //     ctx.GetNuciumEndowment(p)
            //     );
