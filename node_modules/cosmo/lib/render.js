...
var manager = require('./simulationManager');
...
exports.renderSimulation=function(req,res)
{
    ...
    manager.getSimulation(req.name,
        function(err,simulation) 
        {
            if(err) {
                res(err);
                return;
            }
            var response = function(err,colors)
                {
                    if(err) {
                        res(err);
                        return;
            ...
            if(req.date == undefined)
            {
                renderer.render(
                    {
                        ctx: simulation.dates[simulation.dates.length-1],
                        mode: req.mode
                    },
                    response);
            }
...
exports.render = function(req,res)
{
			...
			else
			{
				...
				colors[z] = cmykToHex(
					colorizeVegetationAndHeight(
						numberPlants,//Math.floor(Math.random()*9),
	...
	else if (req.mode == "Nutro")
	{
		for(var z=0; z < req.ctx.area; z++)
		{
			...
			{
				colors[z] = cmykToHex(colorizeNutro(req.ctx.nutro[z],req.ctx));
			}
		}
	}
	else if (req.mode == "Nucium")
	{
		for(var z=0; z < req.ctx.area; z++)
		{
			...
			{
				colors[z] = cmykToHex( colorizeNucium(req.ctx.nucium[z],req.ctx));
			}
		}
	}
	else if (req.mode == "Nutrients")
	{
		for(var z=0; z < req.ctx.area; z++)
		{
			...
			{
				colors[z] = cmykToHex(colorizeNutrients(req.ctx.nutro[z], req.ctx.nucium[z],req.ctx));
			}
		}
	}
	else if (req.mode == "Sunlight")
	{
		for(var z=0; z < req.ctx.area; z++)
		{
			colors[z] = cmykToHex(colorizeSunlightAndHeight(req.ctx.sunlight[z],req.ctx.height[z],req.ctx));
		}
	}
	else if (req.mode == "Rainfall")
	{
		for(var z=0; z < req.ctx.area; z++)
		{
			colors[z] = cmykToHex(colorizeRainfallAndHeight(req.ctx.rainfall[z],req.ctx.height[z],req.ctx));
		}
	}
	else {
